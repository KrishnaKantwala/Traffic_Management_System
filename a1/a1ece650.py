...
def change(st, co, data_sheet):
    if len(data_sheet) == 0:
        ...
        for key in data_sheet.keys():
            if key == str(st[0]).lower():
                data_sheet.update(dict(zip(st, co)))
                print("updated data sheet: ", data_sheet)
                return data_sheet
...
def generate_graph(data_sheet):
    ...
    if len(match_intersaction_list) == 0:
        ...
        for n in range(0, len(match_intersaction_list)):
            value3 = match_intersaction_list[n][0]
            for m in range(0, len(intersaction_list)):
                value4 = intersaction_list[m][0]
                if value3 != value4:
                    diff_intersaction_list.append((value4, intersaction_list[m][1]))
    ...
    for b in range(0, len(list1)):
        ...
        for c in range(b + 1, len(list1)):
            ...
            if val3 == val4:
                if val1 > val2:
                    edge_list.append((val3[0], val2))
                    edge_list.append((val3[1], val1))
                    edge_list.append((val2, val1))
                else:
                    edge_list.append((val3[0], val1))
                    edge_list.append((val3[1], val2))
                    edge_list.append((val2, val1))
...
def main():
    ...
    data_sheet = {}
    while True:
        print("Enter your choice for street : add (a or A) | change (c or C) | remove (r or R) | generate_graph (g or G)")
